...
public class TickerProducer {
    ...
    public static void main(String[] args) {
        ...
        sendMessages(producer, generator);
    ...
    private static TickerGenerator createGenerator() {
        TickerGenerator generator = TickerGenerator.builder()
    ...
    private static void sendMessages(Producer<String, TickerMessage> producer, TickerGenerator generator) {
        for (int i = 0; i < MESSAGE_COUNT; i++) {
            TickerMessage tickerMessage = generator.next();
            sendMessage(producer, tickerMessage, i);
        }
    }

    private static void sendMessage(Producer<String, TickerMessage> producer, TickerMessage message, int index) {
        System.out.printf("Sending ticker message %d: %s%n", index, message.getProductId());
